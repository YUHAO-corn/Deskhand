# 剪贴板智能（Clipboard Intelligence）设计讨论

> 状态：头脑风暴中
> 日期：2026-02-20
> 前置讨论：见 `2026-02-20-screen-aware-assistant-design.md` Q1-Q3（差异化方向探索）
...
在屏幕感知方向的讨论中发现两个问题：
1. "截屏让 AI 回复"跟"手动截图发 ChatGPT"只是省了几步操作，不是真正差异化
2. 真正的差异化（AI 操作电脑）技术不够成熟，Computer Use 准确率不足以面向小白用户

转向剪贴板方向后，最初提出的用户故事（翻译、解释、OCR 等）同样存在问题：
**这些都可以直接发给 ChatGPT 解决，专门做一层"便捷 UI"可能反而带来功能的不通用/不稳定，没有实质价值。**
...
1. **纯自然语言** — 直接说"我之前复制过一个地址"，AI 自己判断。最自然但可能误判。
2. **约定关键词** — 用"剪贴板"等触发词，AI 看到就查历史。更可靠但需要用户知道约定。
3. **输入框快捷操作** — `@clipboard` 或📋按钮，把剪贴板历史作为附件插入对话。最明确但 `@` 语法对小白不直观。
...
1. **全部记录，本地加密存储** — 什么都记，数据只存本地，用系统 Keychain 加密。发给 AI 时才解密。
2. **智能过滤 + 用户可选** — 默认过滤明显敏感内容，提供开关让用户选择"记录所有"。
3. **只记录元数据** — 不存原文，只记录类型/长度/来源/时间。最安全但功能大打折扣。

**决定：方式 1（全部记录，本地加密存储）。**

理由：Deskhand 本身就是能读写文件、执行命令的桌面 agent，用户已经给了很高信任。剪贴板数据的敏感度不会比文件系统更高。本地加密足够。

## Q8: 技术架构——剪贴板监控怎么实现？

参考了两个开源剪贴板管理器：[Maccy](https://github.com/p0deje/Maccy)、[Pesto](https://github.com/matthewpick/pesto-clipboard)。
...
理由：数据已经存在 JSONL 文件里，Agent 已有 Read 工具，AI 完全能自己解析和过滤。专用 Tool 只是把"读文件 + 过滤"封装一层，v1 没必要。
...
v1 scope：
1. 后台剪贴板监控 + JSONL 存储 + 索引
2. 输入框📋按钮 + 剪贴板面板（浏览/搜索/选择）
3. 选中内容作为对话上下文发送
4. 自然语言引用（system prompt 告知文件路径）
...
```
┌──────────────────────────────────────────────┐
│  [弹出面板区域 - Workspace/Tools/Skills 等]    │
├──────────────────────────────────────────────┤
│  文本输入框                                    │
├──────────────────────────────────────────────┤
│  [📎][📁][⭐][🔧][💡][🛡️][🎯]    [👁️][➤]  │
...
有利条件：
- 📎附件按钮已存在但未实现功能——可直接复用
- 弹出面板模式已有先例（Workspace/Tools/Skills 都是点按钮弹面板）
- 附件数据模型已定义（`StoredAttachment`，支持图片/文本/PDF）
...
### 面板设计（v2 — 2026-02-24 重新设计）

> 原设计是纵向列表 + emoji 图标，实际验收发现 4 个问题：
> 1. 📎按钮被剪贴板独占，文件附件功能丢失
> 2. emoji 图标不专业
> 3. 固定 340px 宽度太窄，内容预览被截断
> 4. 纵向列表无法合理展示图片缩略图
...
│ │ [文档图标]         │ │ [链接图标]         │ │
│ │ Dear John,        │ │ https://          │ │
│ │ regarding the     │ │ github.com/       │ │
│ │ proposal we...    │ │ p0deje/Maccy      │ │
│ │                   │ │                   │ │
│ │ 3m · 1.8k 字符    │ │ 8m · 链接          │ │
│ └───────────────────┘ └───────────────────┘ │
│ ┌───────────────────┐ ┌───────────────────┐ │
...
- 类型图标：SVG 线条图标（跟工具栏风格统一），不用 emoji
  ...
  - link → 链接图标
  - image → 图片/风景图标
- 文本卡片：多行预览（不只一行截断），底部显示时间 + 字符数
- 图片卡片：显示缩略图，跟文本卡片等高，底部显示时间 + 文件大小
- 链接卡片：显示 URL 预览
...
探讨了三种方式：
1. **📎按钮动态 badge** — 显示"已记录 N 条"，数字变化时闪一下。复用现有 UI 模式。
2. **新复制时 toast** — 每次新复制在角落闪一个"已记录"提示。更明显但打扰。
3. **不做提示** — 用户打开📎面板自然看到历史。

**决定：方式 3（不做提示）。**

理由：后台功能不需要不断刷存在感。就像 Time Machine 不会一直告诉你"已备份 N 个文件"。用户知道它在跑就行，需要时打开就能找到。
...
Deskhand 记住你复制过的一切，随时可以用自然语言找回或让 AI 批量处理。

### v1 Vertical Slice 开发计划

**Slice 1：能记录、能看到**
- 主进程：剪贴板轮询监控（0.5s）+ JSONL 存储
- 前端：📎按钮点击弹出面板，展示剪贴板历史列表（只读，不可选）
- 验收：复制几段文字，打开面板能看到记录

**Slice 2：能选择、能发送**
- 面板支持点击选中（多选）+ 确认附加
- 选中内容作为附件出现在输入框上方
- 发送时内容内联到消息中（短文本全贴，长文本截断 500 字符 + 引用）
- 验收：选两条剪贴板记录，附加到消息里发送，AI 能看到并回应

**Slice 3：能搜索、能找回**
- 面板顶部搜索框，关键词过滤
- 索引文件（clipboard-index.jsonl）生成
- 验收：复制 20+ 条内容后，搜索关键词能快速定位

**Slice 4：自然语言引用**
- Agent system prompt 中注入剪贴板历史文件路径
- AI 在用户提到"剪贴板"/"我复制过"时自动 Read 索引 → 定位 → 读取完整内容
- 验收：不打开面板，直接对话"帮我找之前复制的那个链接"，AI 能找到

**Slice 5：图片支持 + 收尾**
- 支持记录剪贴板中的图片（单独存文件，JSONL 存路径）
- 面板中图片显示缩略图，发送时用原图
- 自检测标记、重复去重、存储上限 500 条清理
- 首次引导提示
- 验收：完整端到端体验，文本 + 图片都能记录、浏览、选择、发送、自然语言找回

### v1 实现清单（按模块）

**后端（主进程）：**
- 剪贴板轮询监控（0.5s 间隔，内容哈希比对）
- JSONL 存储（完整内容）+ 索引文件（元数据 + 预览）
- 本地加密（系统 Keychain）
- 自检测标记（避免 Deskhand 自身写入触发记录）
- 重复检测（哈希去重）
- 存储上限 500 条，超出清理最早的

**前端（渲染进程）：**
- 复用📎按钮，点击弹出面板（"文件" / "剪贴板" 两个 tab）
- 剪贴板面板：纵向列表 + 搜索框 + 多选 + 确认附加
- 选中内容作为附件显示在输入框上方
- 首次使用引导提示

**Agent 集成：**
- System prompt 中告知剪贴板历史文件路径
- AI 通过现有 Read 工具直接读取 JSONL / 索引文件
- 内容策略：短文本全贴、长文本截断 500 字符 + 引用、图片原图最多 5 张

---

## Q13: 认知转变——从"独立功能"到"透明能力扩展"（2026-02-24）
...
1. **抽象"透明数据源"模式**：让添加新的环境数据源（文件、浏览器历史等）变得简单
2. **优化 System prompt**：不是告诉 AI "你有剪贴板功能"，而是"你可以访问用户的环境数据，在需要时自行调用"
3. **面板 UI 保持现状**：已做的不删，但不再投入更多精力打磨。定位从"剪贴板管理器"变成"附件选择器的一个 tab"
4. **Clip Collect / Clip Insight 重新评估**：在"透明能力"框架下，这些可能不需要专门的 UI，而是 AI 在合适语境下自然触发的行为
