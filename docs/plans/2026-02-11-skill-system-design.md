...
**深度用户（红海）**：知道 skill 是什么，会自己获取，也知道主动让 AI 去 find。这批人用运营手段解决（文档、推荐优质源、社区分享），不需要产品投入。

**浅层用户（蓝海）**：不知道也不关心 skill。他们只会说"帮我做 XX"。这批人才是产品要服务的对象。产品设计应面向这群人。

### 候选方案分析

**1. 内置精选 Skills（开箱即用）** ✅ 确定要做
- 打包精选 SKILL.md 进 app，作为默认配置
- 服务对象：浅层用户——遇到通用常见问题时 AI 自然使用，用户无需知情
- 工程量：低，增加 skill 作为默认配置即可

**2. Agent 自主获取（用户无感）** ❌ 已排除
- 排除理由：安全风险；skill 元数据膨胀会降低 AI 判断质量（选择困难）

**3. 内置 find-skills（Agent 搜索+用户确认）** ✅ 确定要做
- 将 find-skills 作为默认配置的 skill 之一
- 服务对象：深度用户——他们知道可以让 AI 搜索 skill
- 参考：https://github.com/vercel-labs/skills/blob/main/skills/find-skills/
- 工程量：低，将现成的 find-skills 加入默认配置

**4. 应用内 Skill 商店** ❌ 已排除
- 排除理由：软件时代应用商店思路，用户认知负担没变，不是最佳解

**5. Skill Recommend（行为分析推荐）** 🔄 需进一步讨论
- 洞察用户行为 → 推荐已有 skill
- 与 6 是同一系统的两个解法，需要头脑风暴明确

**6. Skill Auto-Create（自动生成）** 🔄 需进一步讨论
- 洞察用户行为 → 自动生成新 skill
- 与 5 是同一系统的两个解法，需要头脑风暴明确

### 5 和 6 的关系

5 和 6 本质上是同一个系统的两面——都是洞察用户行为，区别在于输出：
- 5 输出：推荐一个已存在的 skill
- 6 输出：生成一个全新的 skill

需要进一步头脑风暴来明确：推荐好还是自动生成好，或者两者如何结合。
