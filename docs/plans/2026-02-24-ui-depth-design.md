...
原始状态：标题栏用 `bg-secondary`（略浅的白）+ `border-b` 和聊天区分开。

但标题栏和聊天区本质上是同一个"地面"——它们都是背景层。真正需要"浮起来"的是侧边栏和输入框，它们才是放在地面上的积木。

改动：标题栏改为 `bg-primary`（和聊天区同色），去掉 border 和 shadow。现在标题栏和聊天区融为一体，形成一个连续的青色画布。
...
侧边栏底部有一个操作栏（`···` 和 `Update`），原来用 `border-top` 和会话列表分开。但侧边栏是一张卡片，一张卡片就是一个面。在一个面的内部画线，等于在说"这其实是两个东西"——那它就不是一张卡片了。
...
原始状态：`border border-[var(--border-light)]` + `shadow-[0_2px_8px_rgba(0,0,0,0.04)]`。

border 和 shadow 同时存在时，它们在传达矛盾的信号：
- border 说："我的边界在这里"（二维）
- shadow 说："我浮在上面"（三维）

去掉 border，只留 shadow（并加强到 `0.08` / `16px`），信号变得统一：**这是一个浮在聊天内容上方的白色输入卡片。**
...
1. **新增 UI 元素时先问：它在哪一层？** 确定 z 轴位置后，用对应强度的阴影。不要默认加 border。
2. **同一层的元素共享背景色**，不需要边框分隔。用间距和留白来区分。
3. **圆角 + 间距 = 独立物体**。如果一个元素需要看起来是"放在那里的"，给它圆角和四周的呼吸空间。
4. **阴影强度对应层级高度**。Layer 1 用 `0.06`，Layer 2 用 `0.08`，弹窗用 `0.12`。保持一致。
5. **border 是最后手段**，只用在表单控件（input/select）和数据表格上。
...
| Commit | 改动 |
|--------|------|
| `1c0e87a` | sidebar: 浮动卡片化（margin + rounded + shadow，去掉 border-r/border-t，选中态加阴影） |
| `7e63e4c` | titlebar: 融入主画布（bg-primary，去掉 border-b） |
| `89079fc` | input toolbar: 去掉 border，加强 shadow |
