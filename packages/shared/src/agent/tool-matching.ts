/**
 * Tool Matching - SDK 消息到 AgentEvent 的无状态转换
 *
 * 核心职责：从 Claude Agent SDK 的消息中提取 tool_start 和 tool_result 事件
 *
 * 设计原则：
 * - 无状态：每次输出仅依赖当前消息 + append-only 的工具索引
 * - 直接 ID 匹配：利用 SDK 提供的 tool_use_id 直接关联，无需 FIFO 队列
 * - 顺序无关：工具注册顺序不影响最终结果
 *
 * SDK 提供的关键字段：
 * - parent_tool_use_id: 标识子代理上下文（Task ID 或 null）
 * - tool_use_id: 直接标识工具结果属于哪个工具调用
 */

import type { AgentEvent } from '@deskhand/core/types';

// ─────────────────────────────────────────────────────────────────────────────
// ToolIndex - 工具元数据索引（append-only）
// ─────────────────────────────────────────────────────────────────────────────
